...
│ └──────────────────────────────────────┘   │
│ ┌─ 現在コピー中（アクティブコピー） ──────────┐   │
│ │ 7xKX…AsU   スコア 78 (B)  ✅COPY_OK       │   │
│ │ 保有33日 | 勝率56% | 未実現±0 | スイング安定型│   │
│ │ [エンジン評価の詳細][🔄再評価][変更][解除]  │   │
│ └──────────────────────────────────────┘   │
│ ┌─ 💰 ポジションサイジング・アドバイザー ────┐   │
│ │ 残高3.412 SOL×リスク5%÷SL60%×gradeB係数0.8│   │
│ │ 0.284 SOL   現在ルール: 0.3 SOL (-0.016)  │   │
│ └──────────────────────────────────────┘   │
│ ┌─ 運用ルール ────────────────────────────┐   │
│ │ 1回 0.3 SOL | SL -60% | リスク5%/回      │   │
│ │ Dev Sell 25% / Auto 100%      [編集]    │   │
│ └──────────────────────────────────────┘   │
└──────────────────────────────────────────────┘
```

**アクティブコピー機能（DESIGN.md初版からの追加）:**
GMGN上で実際にコピー中のトレーダーのウォレットアドレスを登録すると（`gmgn.activeCopy.v1`）、`saveTrader()` を通じて既存の Discovery/Validation と同じ保存・アラート基盤に載る。「再評価」ボタンで RPC を再取得し、前回スコアからの急落・戦略変化・NG化を Risk Panel に通知する。未登録時は固定サンプル値（Game・33日保有）を表示し、機能を体験できる状態にする。RPC取得に失敗しても手動データのみで登録は完了する（Validationタブの `fetchRpc` と同じフォールバック方針）。

**ポジションサイジング・アドバイザー（`src/engine/sizing.js`）:**
「勝率より保有時間」と同じ思想で、**トレーダーの品質が低いほど賭け金を絞る**。

```
リスク額(SOL) = 残高 × リスク%（運用ルールで設定、既定5%）
提案額(SOL)   = リスク額 ÷ |SL%| × グレード係数
グレード係数: A=1.0 / B=0.8 / C=0.5 / D=0（コピー非推奨）
              評価未確定（INSUFFICIENT_DATA）の場合も 0
              （品質不明のままフルサイズを提案しない）
```

残高がクリティカル閾値（1 SOL、BalanceTabの `crit` 判定と共通）を下回る場合や、提案額が手数料負けリスクのある最小額（0.1 SOL、LESSONSの教訓と共通）を下回る場合は `warnings` に理由を明記する。
...
| `gmgn.rules.v1` | `{buySOL:0.3, slPct:-60, tpPct:null, devSellPct:25, autoSellPct:100, riskPerTradePct:5}` | 運用ルール |
| `gmgn.alerts.v1` | `Alert[]` | Risk Panel 用 |
| `gmgn.activeCopy.v1` | `{address: string\|null}` | GMGN上で現在コピー中のトレーダーのアドレス。設定すると Home の再評価・Risk Panel 監視対象になる |
...
6. RPC推計値が2/4項目のみでも `dataCompleteness` は実測値を反映し（一律0%にならず）、3/4項目（保有時間・勝率・取引頻度が揃う）ならスコアが完走すること
